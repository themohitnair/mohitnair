...
const Navbar: React.FC<NavbarProps> = ({ links }) => {
    ...
    return (
        <header className="top-0 sticky flex justify-between items-center px-7 py-5 bg-gradient-to-r from-blue-900 via-purple-500 to-indigo-500 bg-opacity-85 z-50">
            ...
            {isMenuOpen && (
                <div className="flex flex-col md:hidden items-end absolute top-full left-0 right-0 bg-gradient-to-r from-blue-900 via-purple-500 to-indigo-500 bg-opacity-75 py-3 px-7 space-y-3 z-50 text-xl sm:text-2xl">
                    {links.map((item, key) => (
                        <Navlink text={item.text} link={item.link} key={key} />
                    ))}
                </div>
